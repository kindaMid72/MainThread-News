...
export default function CategoryPage({articles, count}: {articles: ArticleQuery[], count: number}) {
    ...
    const fetchArticles = async (page: number) => {
        try {
            setLoading(true);
            const response = await api.get(`/api/public/get-all-categories/${categoryName}`);
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 min-h-screen">
            <h1 className="text-4xl font-bold text-gray-900 mb-12">{categoryName}</h1>
